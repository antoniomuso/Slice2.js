...
describe('nano-slice', () => {
  ...
  test('3d array', () => {
    ...
    expect(slice(arr)['1'][1]).toEqual([1, '2', 3, '4', 5, '6', 7, '8', 9, '0']);
    expect(slice(arr)[1][1]).toEqual([1, '2', 3, '4', 5, '6', 7, '8', 9, '0']);

    expect(slice(arr)[0]['-2']).toEqual([1, '2', 3, '4', 5, '6', 7, '8', 9, '0']);
    expect(slice(arr)[0][-2]).toEqual([1, '2', 3, '4', 5, '6', 7, '8', 9, '0']);

    expect(slice(arr)[0][1]['0:3']).toEqual([1, '2', 3]);
    expect(slice(arr)[0][1]['2:5']).toEqual([3, '4', 5]);

    expect(slice(arr)[0][1][':2']).toEqual([1, '2']);
    expect(slice(arr)[0][1][':-2']).toEqual([1, '2', 3, '4', 5, '6', 7, '8']);

    expect(slice(arr)[0][1]['2:']).toEqual([3, '4', 5, '6', 7, '8', 9, '0']);
    expect(slice(arr)[0][1]['-2:']).toEqual([9, '0']);

    expect(slice(arr)[0][1][':']).toEqual([1, '2', 3, '4', 5, '6', 7, '8', 9, '0']);

    expect(slice(arr)[0][1]['::3']).toEqual([1, '4', 7, '0']);
    expect(slice(arr)[0][1]['1:5:2']).toEqual(['2', '4']);

    expect(slice(arr)[0][1]['-5:-1']).toEqual(['6', 7, '8', 9]);
    expect(slice(arr)[0][1]['-5:-1:2']).toEqual(['6', '8']);
    expect(slice(arr)[1][1]['5:1:-2']).toEqual(['6', '4']);
    expect(slice(arr)[0][1]['-1:0:-1']).toEqual(['0', 9, '8', 7, '6', 5, '4', 3, '2']);

    expect(slice(arr)[0][1]['-1:0:-1']['2:5']).toEqual(['8', 7, '6']);
    expect(slice(arr)[0][1]['-5:-1:2']['::']).toEqual(['6', '8']);
    expect(slice(arr)[0][1]['-5:-1']['::-1']).toEqual([9, '8', 7, '6']);


    expect(slice(arr)[0][1]['-1:-5']).toEqual([]);
    expect(slice(arr)[0][1]['-5:-1:-1']).toEqual([]);

    expect(slice(arr)[0][1][':20']).toEqual([1, '2', 3, '4', 5, '6', 7, '8', 9, '0'])
